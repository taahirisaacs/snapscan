...
add_filter( 'woocommerce_payment_gateways', 'add_card_class' );

function add_card_class( $gateways ) {
    $gateways[] = 'WC_Card_Gateway';
    return $gateways;
}

add_action( 'plugins_loaded', 'init_card_class' );

function prefix_register_card_rest_routes() {
    $controller = new WC_Card_Gateway();
    $controller->register_routes();
    $controller->register_cron();
}

add_action( 'rest_api_init', 'prefix_register_card_rest_routes');

function init_card_class()
{
    class WC_Card_Gateway extends WC_Payment_Gateway{
        ...
        public function __construct()
        {
            ...
            $this->method_title = 'Card Payments - powered by SnapScan';
            $this->method_description = 'Accept card payments through entering card details. (Powered by SnapScan)';
            ...
            $this->card_endpoint = 'https://pos.snapscan.io/qr/';
            ...
            add_action('woocommerce_receipt_card', array($this, 'payment'));
        ...
        public function check_payments(){
            ...
            foreach ($body as $payment){
                $order = new WC_Order((int) $payment->merchantReference);
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                ...
                'description' => array(
                    ...
                    'default' => 'Pay using Visa or Mastercard.',
                ),
                'merchant_id' => array(
                    'title' => 'Merchant ID',
                    'type' => 'text'
        ...
        public function order_redirect(){
            $snap = $_GET['merchant_reference'];

            $check_order_url = $this->api_endpoint;

            if (strrpos($check_order_url, '?') === false) {
                $check_order_url .= '?snapCodeReference=' . $snap;
            } else {
                $check_order_url .= '&snapCodeReference=' . $snap;
            }
        ...
        public function payment($order_id)
        {
            ...
            $order_redirect_url = site_url(null, 'https') . '/wp-json/snap/s';

            $user = wp_get_current_user();
            $email = $user->user_email;

            if($email == "") {
                $email = WC()->customer->billing_email;
            }

            $total_in_cents = round($order->get_total() * 100);
            $snapcode = $this->settings['merchant_id'];
            $qr_card_url = 'https://pos.snapscan.io/qr/' . $snapcode . '?id=' . $order_id . '&amount=' . $total_in_cents . '&pm=card'
            . '&imm=true' . '&r_url=' . $order_redirect_url . '&email=' . $email;
		...
		<div class="card-wrapper">
			...
			<div id="card-widget" style="margin:0 auto;text-align: center; border:none">
			    <a href="'.$qr_card_url.'" target="_blank">Proceed to Payment</a>
			</div>
			...
			<script type="text/javascript">
              var HTML_ENTITIES = {
				...
				function redirectPayment() {
					location.replace(htmlDecode("'.$qr_card_url.'"));
				}
				redirectPayment();
			</script>';
